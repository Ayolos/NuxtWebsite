<template>
  <div ref="contactSection" class="flex-col items-center py-20 lg:py-24 h-max w-full lg:px-20 sm:px-10 px-4 2xl:w-[1500px] 2xl:mx-auto flex gap-10">
    ...
    <div class="flex lg:flex-row flex-col-reverse h-full xl:w-3/4 w-full">
        ...
        <form @submit.prevent="envoyerMail">
          <div class="flex flex-col gap-4">
            <div class="grid grid-cols-2 gap-8 max-sm:grid-cols-1">
              <InputForm v-model="nom" type="input" value="nom">Nom</InputForm>
              <InputForm v-model="prenom" type="input" value="prenom">Prénom</InputForm>
            </div>
            <InputForm v-model="email" type="input" value="email">Adresse mail</InputForm>
            <InputForm v-model="message" type="textarea" value="message">Message</InputForm>
          </div>
          <div class="flex flex-row gap-4 h-full items-center pt-4">
            <button
                class="flex border border-purple-500 items-center text-purple-400 rounded-lg px-4 gap-1 py-2 bg-purple-950 hover:bg-purple-900">
              Envoyer
            </button>
            <UIcon v-if="isMailLoading" class="text-3xl text-purple-500" name="eos-icons:bubble-loading"></UIcon>
            <icon-style v-else-if="!isMailLoading && isMailSent" padding="1.5">
              <UIcon class="h-7 w-7 text-green-500" name="line-md:email-check-twotone"></UIcon>
            </icon-style>
            <icon-style v-else-if="!isMailLoading && isMailError" padding="1.5">
      ...
      <div
          class="z-0 p-10 border-t border-l border-r md:border-t lg:border-b lg:border-r lg:border-l-0 border-slate-800
               ...
               basis-2/5 lg:rounded-r-xl max-lg:rounded-t-xl shadow-2xl relative bg-background-light
        ...
        <icon-text icon="ic:twotone-phone">
          ...
          <template #content>07 78 19 27 97</template>
        </icon-text>
        <icon-text icon="ic:twotone-email">
          <template #label>Email</template>
          <template #content>ayolos14@gmail.com</template>
        </icon-text>
        <icon-text icon="ic:twotone-place">
          <template #label>Lieu</template>
          <template #content>Lille</template>
        </icon-text>
...
const contactSection = ref(null); // 👉 Référence pour GSAP
